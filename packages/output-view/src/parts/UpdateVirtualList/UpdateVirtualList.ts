...
export const updateVirtualList = (state: OutputState, requestedDeltaY?: number): OutputState => {
  const { deltaY: currentDeltaY, filteredItems, followOutput, height, itemHeight, scrollLockEnabled } = state
  ...
  const requested = requestedDeltaY ?? (followOutput && !scrollLockEnabled ? finalDeltaY : currentDeltaY)
